...
subtitle: "From MLE to Full Bayes — each level trades optimization for integration."
description: "Multiple levels of data analysis go from Frequentist to more and more Bayesian, starting from MLE and going to full Bayes via Type-2 MLE (Empirical Bayes). The outstanding observation is the increasing reliance on integration as we get more Bayesian."
categories:
    - bayesian
    - statistics
date: 2025-05-28
...
This is the first technique we learned and used. There is no notion of priors here, just a likelihood $\cal{L}$ which we maximize (or equivalently minimize cost $-\ell = - log(\cal{L})$ .
